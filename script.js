...
function openSite() {
    ...
    // تشغيل نظام الأغاني وتفعيل الصوت فوراً بعد ضغطة الزرار
    isPlaying = true;
    if (toggleMusicBtn) {
        toggleMusicBtn.textContent = "⏸ Pause";
    }
    
    // تحميل الأغنية الأولى وتشغيلها ميكانيكياً
    loadSong(0);
    if (audioPlayer) {
        audioPlayer.muted = false; // التأكد أن الصوت مش مكتوم
        audioPlayer.play().catch(err => {
            console.log("المتصفح حظر التشغيل التلقائي، سيتم التشغيل عند الضغط على الأزرار:", err);
        });
    }
}
...
// إطلاق القلوب المتساقطة بالخلفية
